...
Upon creation of a new key, you need to update the client ID in the [Chrome Developer Console](https://console.developers.google.com). You then need to update the client ID in the `manifest.json` file as well as add/change the field for the key. In order to find your key, follow Google's instructions [here](https://developer.chrome.com/apps/app_identity#copy_key) or you can find the key using a [crx reader](https://robwu.nl/crxviewer/).
